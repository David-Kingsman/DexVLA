...
class UfactoryDataCollection():
    ...
    def collect_data(self, ip):
        ...
        device = Meta_quest2()
        arm_init_pose = np.asarray(arm.get_position_aa(is_radian=True)[1]) # [x, y, z, rx, ry, rz]
        arm_init_pose_ = Rotation.from_rotvec([arm_init_pose[3], arm_init_pose[4], arm_init_pose[5]], degrees=False)# 3x3 rotation matrix
        ...
        while arm.connected and arm.error_code == 0 and i < self.max_steps:
            ...
            if i % (self.control_frequency / self.demo_save_frequency) == 0:
                ...
                for camera_id in self.camera_ids:
                    ...
                    if cam_interfaces[camera_id].use_color:
                        ...
                        if self.save2memory_first:
                            ...
                            # saving images into disk directly
                ...
                # FT sensor data
                if self.FT_option:
                    code, _ = arm.get_ft_sensor_data() # get the FT sensor data [x, y, z, fx, fy, fz]
                    if code == 0:
