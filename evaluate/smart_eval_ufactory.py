...
def process_obs(obs, states, stats):
    ...
    """
    cur_left_webcam = obs['webcam_1']
    cur_right_webcam = obs['webcam_2']

    assert np.max(cur_left_webcam) > 1, "All images must be 0-255."
    traj_rgb_np = np.array([cur_left_webcam, cur_right_webcam]) # sequential must align with constants.py
